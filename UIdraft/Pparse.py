...
class parse():
    ...
    def setUp(self, text):
            ...
            for sentId, sentence in enumerate(self.doc.sents):
                ...
                for tg in self.tg:
                    ...
                    for i, target in enumerate(piece):
                        ...
                        if piece[i].lower() == tg.gram3.lower() and piece[i - 1].lower() == tg.gram2.lower() and piece[
                            i - 2].lower() == tg.gram1.lower():
                            count += 1
        ...
        def is_exception(target_word, n_gram, gram):
            if n_gram == 1:
                # print("unigram")
                for word_list in inter_values.exception_list:
                    if len(word_list) == 1 and target_word == word_list[0]:
                        return True
                    else:
                        pass
                return False
                ...
                # print("bigram")
                for word_list in inter_values.exception_list:
                    if len(word_list) == 2 and target_word == word_list[gram]:
                        return True
                    else:
                        pass
                return False
                ...
                # print("trigram")
                for word_list in inter_values.exception_list:
                    if len(word_list) == 3 and target_word == word_list[gram]:
                        return True
                    else:
                        pass
                return False        #CONTINUE Shift these return back once


        def checkSafe(self):
            for bigram in self.bg:
                if isStopword(bigram.gram1) or isStopword(bigram.gram2) or \
                        (is_exception(bigram.gram1, 2, 0) and is_exception(bigram.gram2, 2, 1)):
                    bigram.safe = False

            for trigram in self.tg:
                if isStopword(trigram.gram1) or isStopword(trigram.gram2) or isStopword(trigram.gram3) or \
                        (is_exception(trigram.gram1, 3, 0) and is_exception(trigram.gram2, 3, 1) and
                         is_exception(trigram.gram3, 3, 2)):
                    trigram.safe = False
