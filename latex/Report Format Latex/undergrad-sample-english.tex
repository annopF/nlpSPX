...
Referring to figure 3.7, core engine pipeline in chapter 3, we have added a preprocessing step to the Reader component. This stage takes place after the input has been read either from a file or from the input text field. During this process, the following items will be removed from the input text.
...
\end{tabular}
\caption{\label{final vs raw}Comparing final and raw ranking}
\end{table}
...
\end{tabular}
\caption{\label{base vs custom}Comparing base model and finetuned model}
\end{table}
...
OG SEN: original sentence without mask
MASKED WORD: target word to mask
MASKED SEN: sentence replaced the target word with <mask> token
\\\\
Final: the aggregated final ranking using consensus scheme
Anotator Overlap: words that both annotators selected as appropriate substitutions. This indicates 100% agreement between 2 judges.
%Usable: proportion of words that appear in Annotator Overlap that are also in Final.
Assuming an ideal candidate is the one appears in Annotator Overlap. %Usable metric indicates how many words in the top 15 are considered a good substitution based on human judgment. 

\begin{verbatim}
>>>> OG SEN: The punishment given to the rich people is unsatisfactory to the victim's family.
>>>> MASKED WORD: people
>>>> MASKED SEN: The punishment given to the rich <mask> is unsatisfactory to the victim's family.
\end{verbatim}
\begin{table}[htp]
\begin{tabular}{lllll}
Final &
...
{[}'people', 'folks', 'ones', 'persons', 'individuals', 'classes', 'men', 'citizens', 'things', 'criminals', 'subjects', 'groups', 'leaders', 'masses', 'guys'{]} &
  {[}'people', 'folks', 'ones', 'persons', 'individuals', 'classes', 'men', 'citizens','criminals', 'subjects', 'leaders', 'guys'{]} &
  {[}'people', 'folks', 'ones', 'individuals', 'classes', 'men', 'citizens', 'criminals', 'subjects'{]} &
  \{'subjects', 'individuals', 'folks', 'criminals', 'classes', 'men', 'citizens', 'ones', 'people'\} &
  60
\end{tabular}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
