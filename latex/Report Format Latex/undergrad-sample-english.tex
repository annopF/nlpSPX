...
\noindent{\large\bf  Scenario 3: User Add the overused word to exception list}
\begin{figure}[!h]\centering
\includegraphics[width=15cm]{./img/chp3/Sequence3.png}
\caption{Adding word to exception list sequence diagram}\label{fig:seq3}
\end{figure}

Figure \ref{fig:seq3} shows the process when the user doesn’t want this overused word to not be scanned at all in the document. So, after they click to add this word to the exception list. The program will send the exception word information to the detector to include it in. And also hide the suggestion list in the UI itself.
...
On the other hand, more than half of users are not quite satisfied with the text input shown in the text box, since it’s hard to read or match the position with the original text file. Some of the test users also are not satisfied with the speed of execution which is especially at the suggestion part. They also think that the exclusion list function is still not that useful right now. If the system has options to manage or remove words from the exclusion list, it will be better. As the overall responses result, most of the test users think that the quality of the application is good, and has the potential of helping them in their writing.
...
\begin{figure}[!h]\centering
\includegraphics[width=15cm]{./img/Appendix/FinalvsRerank.png}
\end{figure}

\newpage
\section{Base model vs custom model}
\begin{figure}[!h]\centering
\includegraphics[width=15cm]{./img/Appendix/BasevsCustom.png}
\end{figure}
